 ...
 * reference to the owning entity beyond the entity id used for bookkeeping.</p>
 *
 * <h2>26.1.2</h2>
 * <p>Identical to the 1.20.1 implementation: the whole class is version independent (it only
 * uses {@code Mth}, {@code RandomSource}, {@code Vec3} and JOML), so the two trees share one
 * source of truth for the cloud constants and the lifetime curves.</p>
 */
public final class GasCloud {
    ...
    public List<SubQuad> getSubQuads() {
        List<SubQuad> list = new ArrayList<>(this.subQuads.length);
        for (SubQuad quad : this.subQuads) {
            list.add(quad);
        }
        return list;
    }

    /** Convenience helper for callers that need a JOML vector of the current centre. */
    public Vector3f getPositionVector(float partialTick) {
