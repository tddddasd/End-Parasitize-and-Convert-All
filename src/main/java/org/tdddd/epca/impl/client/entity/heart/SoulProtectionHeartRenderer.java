 ...
 * {@code ADDITIVE_TRANSPARENCY} instead of {@code TRANSLUCENT_TRANSPARENCY}. The reference look is a
 * glowing plasma column over a dark background, and {@code SRC_ALPHA / ONE} makes the near-opaque
 * core add up to a bright near-white glow instead of merely blending towards whatever is behind it.
 * The ordinary translucent variant is therefore left completely untouched for the gas and specks.</p>
 *
...
public final class SoulProtectionHeartRenderer {
    ...
    private static void submitMotes(LivingEntity entity, PoseStack poseStack, MultiBufferSource bufferSource,
        ...
        for (int i = 0; i < state.moteCount; i++) {
            ...
            // Embers are yaw-only billboards as well, so they stay upright like the column.
            GasCloudRenderer.submitVerticalBillboard(entity, poseStack, bufferSource, partialTick,
