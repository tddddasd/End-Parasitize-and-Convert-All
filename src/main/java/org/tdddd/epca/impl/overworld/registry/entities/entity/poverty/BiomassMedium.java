...
public class BiomassMedium extends PathfinderMob implements GeoEntity, IParasite, Enemy {
    private final AnimatableInstanceCache factory = GeckoLibUtil.createInstanceCache(this);
    private static final EntityDataAccessor<Integer> TICK_COUNT = SynchedEntityData.defineId(BiomassMedium.class, EntityDataSerializers.INT);
    ...
    public BiomassMedium(EntityType<? extends PathfinderMob> type, Level level) {
        super(type, level);
        this.xpReward = 0;
    }
    ...
    private void spawnParticles(int currentTick) {
        ...
        if (currentTick == TOTAL_LIFETIME - 2) {
            ...
            for (int i = 0; i < particleCount; i++) {
                ...
                double offsetX = (this.random.nextDouble() - 0.5) * 0.22;
                double offsetY = (this.random.nextDouble() - 0.25) * 0.42;
                double offsetZ = (this.random.nextDouble() - 0.5) * 0.22;
        ...
        if (currentTick == TOTAL_LIFETIME - 1) {
            ...
            int particleCount = 7 + this.random.nextInt(4); 
            ...
            for (int i = 0; i < particleCount; i++) {
                ...
                double randomY = minY + this.random.nextDouble() * (maxY - minY);
                double randomZ = minZ + this.random.nextDouble() * (maxZ - minZ);
                double offsetX = (this.random.nextDouble() - 0.5) * 0.35;
                double offsetY = (this.random.nextDouble() - 0.35) * 0.35;
                double offsetZ = (this.random.nextDouble() - 0.5) * 0.35;
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>("controller", 2, this::predicate));
    }

    private PlayState predicate(AnimationTest<BiomassMedium> event) {
        AnimationController<BiomassMedium> controller = event.controller();
        int currentTick = this.entityData.get(TICK_COUNT);
    ...
    public static boolean checkBiomassMediumSpawnRules(
            EntityType<BiomassMedium> entityType,
