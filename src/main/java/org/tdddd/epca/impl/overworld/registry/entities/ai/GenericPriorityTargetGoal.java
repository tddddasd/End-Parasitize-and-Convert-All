...
public class GenericPriorityTargetGoal extends Goal {
    ...
    public GenericPriorityTargetGoal(Mob mob, double range) {
        this.mob = mob;
    ...
    private LivingEntity getForcedTarget() {
        ...
        try {
            ...
            if (entity instanceof LivingEntity le && le.isAlive() && !isFriendly(le)) {
                return le;
            }
        } catch (IllegalArgumentException ignored) {}
        mob.getPersistentData().remove("forcedTargetUuid");
        return null;
    }
    ...
    private LivingEntity getNearestValidTarget() {
        ...
        List<LivingEntity> candidates = mob.level().getEntitiesOfClass(
                ...
                e -> e != mob && !isFriendly(e) && e.isAlive()
        );
